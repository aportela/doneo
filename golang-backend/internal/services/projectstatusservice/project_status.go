...
func (service *projectStatusService) Get(ctx context.Context, id string) (domain.ProjectStatus, error) {
	...
	if err != nil {
		return projectStatus, fmt.Errorf("[ProjectStatusService] failed to get project status with ID %s: %w", id, err)
	}
