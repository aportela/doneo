...
func (service *taskStatusService) Get(ctx context.Context, id string) (domain.TaskStatus, error) {
	...
	if err != nil {
		return taskStatus, fmt.Errorf("[TaskStatusService] failed to get task status with ID %s: %w", id, err)
	}
