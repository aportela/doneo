...
func (service *projectPermissionService) Add(ctx context.Context, projectId string, permission domain.ProjectPermission) (domain.ProjectPermission, error) {
	...
	err = projecthistoryrepository.NewRepository(service.database).Add(ctx, projectId, domain.HistoryOperation{ID: utils.UUID(), CreatedBy: domain.UserBase{ID: currentUserId}, CreatedAt: time.Now(), OperationType: domain.EventProjectPermissionAdded})
	if err != nil {
		return domain.ProjectPermission{}, err
	}
	err = tx.Commit()
...
func (service *projectPermissionService) Delete(ctx context.Context, projectId string, permissionId string) error {
	...
	err = projecthistoryrepository.NewRepository(service.database).Add(ctx, projectId, domain.HistoryOperation{ID: utils.UUID(), CreatedBy: domain.UserBase{ID: currentUserId}, CreatedAt: time.Now(), OperationType: domain.EventProjectPermissionDeleted})
	if err != nil {
		return err
	}
	return tx.Commit()
